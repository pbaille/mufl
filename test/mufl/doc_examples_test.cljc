(ns mufl.doc-examples-test
  "Tests verifying all examples from doc/introduction.md.
   Merged from doc_verify.clj and doc_examples_test.clj."
  (:require [clojure.test :refer [deftest testing is]]
            [mufl.core :as m]))

;; ════════════════════════════════════════════════════════════════
;; From doc_examples_test.clj (doc- prefix)
;; ════════════════════════════════════════════════════════════════
...
;; ════════════════════════════════════════════════════════════════
;; From doc_verify.clj (verify- prefix)
;; ════════════════════════════════════════════════════════════════

(deftest verify-values
  (is (= [42]      (m/query 42)))
  (is (= ["hello"] (m/query "hello")))
  (is (= [:ok]     (m/query :ok)))
  (is (= [true]    (m/query true))))

(deftest verify-uncertain-values
  (is (= [1 2 3]     (m/query (let [x (one-of 1 2 3)] x))))
  (is (= [1 2 3 4 5] (m/query (let [x (between 1 5)] x)))))

(deftest verify-constraints
  (is (= [3 4 5]
         (m/query (let [x (one-of 1 2 3 4 5)]
                    (and (> x 2) x)))))
  (is (= [2 4]
...
(deftest verify-multiple-variables
  (is (= [[2 4] [1 5]]
         (m/query (let [x (one-of 1 2 3 4 5)
                        y (one-of 1 2 3 4 5)]
                    (and (< x y)
...
(deftest verify-functions
  (is (= [3]
         (m/query (let [double (fn [x] (+ x x))
                        n (between 1 5)]
                    (and (= (double n) 6) n))))))

(deftest verify-recursion
  (is (= [120]
         (m/query (let [fact (fn [n]
                               (if (= n 0) 1 (* n (fact (- n 1)))))]
                    (fact 5)))))
...
(deftest verify-named-constraints
  (is (= [1 2 3]
         (m/query (defn positive [x] (> x 0))
                  (let [n (one-of -1 0 1 2 3)]
                    (positive n)
...
(deftest verify-type-constructors
  (testing "vector-of"
    (is (= [[1 2 3]]
...
(deftest verify-higher-order-functions
  (testing "map"
    (is (= [[2 4 6]]
           (m/query (let [v [1 2 3]
                          doubled (map (fn [x] (* x 2)) v)]
...
(deftest verify-pythagorean-triples
