(ns mufl.destructure-review-test
  "Comprehensive edge-case tests for the destructuring implementation.
   Covers: empty patterns, single-element patterns, deeply nested patterns,
   dot rest edge cases, operator composition, error messages, and
   constraint propagation through destructuring."
...
(deftest vector-head-middle-tail
  ...
  (testing "bare dot in middle: [a . . c] not supported — dot consumes pattern"
    ;; [a . . c] has dot at index 1, after-dot is [. c]
    ;; first after-dot is '.', tail is [c] — '.' as rest-pattern is just a symbol
    ;; This is an edge case; the dot symbol itself gets bound as rest
    ;; which will fail at bind time since '.' is likely not a valid binding target
    ;; We don't explicitly error — it falls through naturally
    ))
...
(deftest or-pattern-different-shapes
  (testing "(or (ks x) [x]) — map fallback to vector"
    ;; map data matches first pattern
    (is (= [42]
           (m/query (let [(or (ks x) [x]) {:x 42}] x))))
    ;; vector data fails ks, falls back to vector destructure
    (is (= [42]
...
(deftest or-destructuring-vs-expression
  (testing "or in destructuring position uses pattern semantics"
    ;; In destructure: (or pat1 pat2) tries each pattern
    (is (= [42]
           (m/query (let [(or (ks x) [x]) {:x 42}] x)))))

  (testing "or in expression position uses domain union semantics"
    ;; In expression: (or expr1 expr2) does domain union
    (is (= #{3 7}
           (set (m/query (let [x (between 1 10)]
                           (and (or (= x 3) (= x 7)) x))))))))

;; ════════════════════════════════════════════════════════════════
;; 20. drop and dissoc primitive edge cases
;; ════════════════════════════════════════════════════════════════
...
;; ════════════════════════════════════════════════════════════════
;; 21. Stress: deeply nested + constraints + as + dot rest
;; ════════════════════════════════════════════════════════════════
...
;; ════════════════════════════════════════════════════════════════
;; 23. Edge case: symbol pattern (non-destructuring) in bind
;; ════════════════════════════════════════════════════════════════
