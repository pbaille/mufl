...
- `if` stores a `:fork` node — deferred to solve time
...
The fundamental issue is that **bind time and solve time have different capabilities**:

| | Bind time | Solve time |
|---|---|---|
...
Recursion needs ground args → ground args come from enumeration → enumeration happens at solve time → but recursion happens at bind time. There's a phase mismatch.

## Approach 1: Solve-time Recursive Unfolding

**Idea**: When solve encounters a fork node that contains a recursive call with a non-ground arg, enumerate the arg's domain and unfold the recursion for each value.
...
3. During solve, when labeling reaches a deferred recursion:
...
- **Nested scopes**: Recursive calls create `call1234` gensym scopes. The solver currently only labels workspace-scope variables, not variables inside call scopes.
...
**Complexity**: High. Requires fundamental changes to the bind/solve boundary.
...
Each branch has a ground arg, so recursion unfolds normally. The cond creates a fork that solve handles.
...
**Advantages**:
- No changes to solver — uses existing fork mechanism
- Recursion still unfolds at bind time with ground args
- The fork branches are pruned by solve like any other fork
